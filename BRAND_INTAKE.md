...
> ⚠️ **NewsAPI 15 词限制**：NewsAPI 免费档单次查询【词数】上限 15（注意是词数不是关键词数，`AI memory`=2 词）。
> 引擎**逐关键词单独查询**（不做 OR 合并 —— 记者反查要作者多样性，OR 合并会被少数高产源垄断、多样性塌掉），
> 单关键词天然 ≤15 词，永不触发该限制。brand processing 时关键词越多 = 查询次数越多（召回更全但更慢，免费档 2000 次/月够用）。
